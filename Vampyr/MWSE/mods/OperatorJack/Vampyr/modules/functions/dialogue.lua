...
local function onDialogueEnvironmentCreated(e)
    ...
    function env.vampyrBloodExtractionTarget(serumId)
        common.logger.debug("Triggering blood extraction for reference: %s; Dialogue/INFO: %s/%s; Into Empty Vial %s", reference, dialogue, info.id, serumId)
    ...
    function env.vampyrEnthrall()
        common.logger.debug("Triggering enthrall on for reference: %s; Dialogue/INFO: %s/%s;", reference, dialogue, info.id)

        enthrall.enthrall(reference)
    end

    function env.vampyrEmbraceTarget()
        common.logger.debug("Triggering embrace on for reference: %s; Dialogue/INFO: %s/%s;", reference, dialogue, info.id)
